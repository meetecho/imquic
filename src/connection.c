	...
	if(conn->socket->qlog_path != NULL) {
		if(conn->is_server) {
			...
			conn->qlog = imquic_qlog_create(conn->name, conn->socket->qlog_sequential,
				TRUE, filename, conn->socket->qlog_quic, conn->socket->qlog_http3,
				conn->socket->qlog_roq, conn->socket->qlog_roq_packets,
				conn->socket->qlog_moq, conn->socket->qlog_moq_messages, conn->socket->qlog_moq_objects);
		} else {
			conn->qlog = imquic_qlog_create(conn->name, conn->socket->qlog_sequential,
				FALSE, (char *)conn->socket->qlog_path, conn->socket->qlog_quic, conn->socket->qlog_http3,
				conn->socket->qlog_roq, conn->socket->qlog_roq_packets,
				conn->socket->qlog_moq, conn->socket->qlog_moq_messages, conn->socket->qlog_moq_objects);
		}
	}
