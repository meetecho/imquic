 ...
 * License: MIT
 *
 * imquic-based MoQ test client for moq-test, see the (expired?) draft
 * https://www.ietf.org/archive/id/draft-afrind-moq-test-01.html for details
 *
 */
...
static imquic_moq_namespace pub_namespace = { 0 };
static char pub_tns_buffer[256];
static const char *pub_tns = NULL;
static uint64_t moq_tns_request_id = 0, moq_track_alias = 0;
static uint8_t relay_auth[256];
static size_t relay_authlen = 0;

static GMutex mutex;
static GHashTable *subscribers = NULL;
...
static int imquic_demo_tuple_to_test(imquic_connection *conn, imquic_moq_namespace *tns, int64_t *test, char *error, size_t error_len) {
	...
	while(temp) {
		if(count >= IMQUIC_DEMO_TEST_MAX) {
			...
			return IMQUIC_MOQ_REQERR_NAMESPACE_TOO_LARGE;
		}
		if(count > 0) {
...
static void imquic_demo_ready(imquic_connection *conn) {
	...
	/* Let's publish our namespace right away */
	IMQUIC_LOG(IMQUIC_LOG_INFO, "[%s] Announcing namespace '%s'\n", imquic_get_connection_name(conn), IMQUIC_DEMO_TEST_NAME);
	...
	imquic_moq_publish_namespace(conn, moq_tns_request_id, &pub_namespace, &params);
}
...
static void imquic_demo_incoming_subscribe(imquic_connection *conn, uint64_t request_id,
	...
	if(res != 0) {
		imquic_moq_reject_subscribe(conn, request_id, res, err, 0, NULL);
		return;
	}
	g_mutex_lock(&mutex);
...
static void imquic_demo_incoming_standalone_fetch(imquic_connection *conn, uint64_t request_id,
	...
	int res = imquic_demo_tuple_to_test(conn, tns, test, err, sizeof(err));
	if(res != 0) {
		imquic_moq_reject_fetch(conn, request_id, res, err, 0, NULL);
		return;
	}
	...
	if(range->start.group > largest.group || range->end.group < (uint64_t)test[TUPLE_FIELD_START_GROUP] ||
		...
		imquic_moq_reject_fetch(conn, request_id, IMQUIC_MOQ_REQERR_INVALID_RANGE, "FETCH range outside of test range", 0, NULL);
		return;
	}
	g_mutex_lock(&mutex);
	/* Create a subscriber, if needed */
...
static void imquic_demo_incoming_joining_fetch(imquic_connection *conn, uint64_t request_id, uint64_t joining_request_id ,
	...
	imquic_moq_reject_fetch(conn, request_id, IMQUIC_MOQ_REQERR_NOT_SUPPORTED, "Not implemented yet", 0, NULL);
}
...
int main(int argc, char *argv[]) {
	...
	pub_namespace.buffer = (uint8_t *)IMQUIC_DEMO_TEST_NAME;
	pub_namespace.length = strlen(IMQUIC_DEMO_TEST_NAME);
	pub_namespace.next = NULL;
	pub_tns = imquic_moq_namespace_str(&pub_namespace, pub_tns_buffer, sizeof(pub_tns_buffer), TRUE);
	IMQUIC_LOG(IMQUIC_LOG_INFO, "Using namespace '%s'\n", pub_tns);
	...
	imquic_server *client = imquic_create_moq_client("moq-test",
		IMQUIC_CONFIG_INIT,
	...
	default_test[TUPLE_FIELD_PROTOCOL] = 0;					/* Ignored, this will always be "moq-test-00" */
