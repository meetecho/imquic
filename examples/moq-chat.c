...
static imquic_demo_moq_participant *imquic_demo_moq_participant_create(imquic_connection *conn, const char *name, uint64_t request_id);
...
static void imquic_demo_incoming_unannounce(imquic_connection *conn, imquic_moq_namespace *tns) {
	...
	IMQUIC_LOG(IMQUIC_LOG_INFO, "[%s] Namespace unannounced: '%s'\n",
		imquic_get_connection_name(conn), ns);
	/* Find the request_id associated to that namespace, and unsubscribe */
	g_mutex_lock(&mutex);
	imquic_demo_moq_participant *p = g_hash_table_lookup(participants_byns, ns);
	if(p != NULL) {
		imquic_moq_unsubscribe(conn, p->request_id);
		g_hash_table_remove(participants_byns, ns);
		g_hash_table_remove(participants, &p->request_id);
	}
	g_mutex_unlock(&mutex);
}
