...
gboolean demo_options_parse(demo_options *options, int argc, char *argv[]) {
	...
	GOptionEntry opt_entries[] = {
		...
		{ "bidirectional-stream", 'B', 0, G_OPTION_ARG_NONE, &options->bidi, "Whether to send the text on a bidirectional STREAM (default=yes, if no other option is provided)", NULL },
		{ "unidirectional-stream", 'U', 0, G_OPTION_ARG_NONE, &options->uni, "Whether to send the text on a unidirectional STREAM (default=no)", NULL },
		{ "datagram", 'D', 0, G_OPTION_ARG_NONE, &options->datagram, "Whether to send the text on a DATAGRAM (default=no)", NULL },
		...
		{ "qlog-logging", 'l', 0, G_OPTION_ARG_STRING_ARRAY, &options->qlog_logging, "Save these events to QLOG (can be called multiple times to save multiple things; default=none)", "quic|http3" },
