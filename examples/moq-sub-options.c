...
gboolean demo_options_parse(demo_options *options, int argc, char *argv[]) {
	...
	GOptionEntry opt_entries[] = {
		...
		{ "test-range-filter", '7', 0, G_OPTION_ARG_NONE, &options->test_filter_ranges, "Send some test filter ranges in the SUBSCRIBE (default=no)", NULL },
		...
		{ "update-subscribe-namespace", 'U', 0, G_OPTION_ARG_INT, &options->update_subscribe_namespace, "When using SUSBCRIBE_NAMESPACE, send a REQUEST_UPDATE after a few seconds for the 'test-update' prefix (default=-1, never change prefix)", "seconds" },
